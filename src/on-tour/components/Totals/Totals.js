...
import { adjustDistance } from '../../helpers/helpers'
...
export default class Totals extends Component {
      ...
      return (
        <div className="mpg-edit-container">
          <button className="add-gig-info-exit"
                  onClick={() => {this.exitMPGEdit()}}>&times;</button>
  ...
  updateLocations() {
    ...
    Promise.all(distanceMap).then(distances => {
        this.props.handleUpdateGigList(
          this.updateLocationMap(distances)
        )
        this.props.handleSetCheck(true)
        this.setState({totalMiles:
                          this.totalMiles(
                            this.updateLocationMap(distances)
                          ),
                       totalCost:
  ...
  updateLocationMap(distances) {
    ...
    return Locations.map((updateLocation, i) => {
      if(i === 0) {
        ...
        updateLocation.distance_from_last = parseInt(distances[i - 1], 10).toFixed(1)
        updateLocation.cost_from_last =
          (parseInt((distances[i - 1]), 10) / this.props.MPG * updateLocation.gasPrice).toFixed(2)
        return updateLocation
      }
    })
  }
  ...
  totalMiles(locationArray) {
    const totalMiles = parseInt(locationArray.reduce((distanceAcc, gig) => {
        return distanceAcc + parseInt(gig.distance_from_last, 10)
      }, 0), 10)
        ...
        <div>
          <div id='totals'>
            ...
            <div className="totals-card">
              {this.editMPG()}
              <h6 className="totals-card-mpg total"
                  onClick={() => this.toggleMPGEdit()}>{this.props.MPG}</h6>
              ...
              <h6 className="totals-card-print total" >
                <a className="totals-card-print-link"
                    href="../Print/print.html"
                    target='_blank'>
                <img className="print-icon" alt="print icon" src="printer.svg" />
                </a>
              </h6>
            </div>
          </div>
